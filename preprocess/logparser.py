...
class LCSObject:
    """ Class object to store a DeepLog group with the same template
    """
...
class LogParser:
    ...
    def parse(self, logname):
        ...
        for idx, line in tqdm(self.df_log.iterrows(), total=self.linecount):
            ...
            # Find an existing matched DeepLog cluster
            ...
            if matchCluster is None:
                ...
                if matchCluster is None:
                    ...
                    # Match no existing DeepLog cluster
                    if matchCluster is None:
                        newCluster = LCSObject(logTemplate=logmessageL, logIDL=[logID])
                        logCluL.append(newCluster)
                        self.addSeqToPrefixTree(rootNode, newCluster)
                    # Add the new DeepLog message to the existing cluster
                    else:
    ...
    def log_to_dataframe(self, log_file, regex, headers, logformat):
        """ Function to transform DeepLog file to dataframe
        """
        log_messages = []
        self.linecount = 0
    ...
    def generate_logformat_regex(self, logformat):
        """ Function to generate regular expression to split DeepLog messages
        """
        headers = []
